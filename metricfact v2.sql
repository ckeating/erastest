...
SELECT --median los proc to disch
	...
	FROM	(
        SELECT  e.CSN
        ,       e.Discharge_DateKey
        ,       DATEDIFF(DAY, c.procedurefinish, e.Discharge_DTTM) AS proclos
        ,       e.ERASEncounter
        ,       OrdersetFlag
		,		e.[Admission Type]
        ,       e.OpenVsLaparoscopic
        ,       c.SurgeonName
...
SELECT --avg los proc to disch
	   ...
	   ,1 'Den'
	FROM	(
		SELECT  e.csn, e.Discharge_DateKey,DATEDIFF(DAY,c.procedurefinish, e.Discharge_DTTM) AS proclos,e.ERASEncounter,e.OrdersetFlag,e.OpenVsLaparoscopic,c.SurgeonName,[Admission Type]
		FROM radb.dbo.vw_CRD_ERAS_GHGI_Case AS c
		LEFT JOIN radb.dbo.vw_CRD_ERAS_GHGI_EncDim e ON c.AdmissionCSN=e.CSN
		) AS c
...
SELECT --% liquids 2 hrs before induction
	...
	FROM
		 ...
		 LEFT JOIN radb.dbo.vw_CRD_ERAS_GHGI_EncDim AS enc ON enc.CSN=ah.AdmissionCSN
