...
IF object_id('RADB.dbo.CRD_ERAS_Case_GHGI') IS NOT NULL
	DROP TABLE RADB.dbo.CRD_ERAS_Case_GHGI;
...
INTO RADB.dbo.CRD_ERAS_Case_GHGI
...
UPDATE radb.dbo.CRD_ERAS_Case_GHGI
...
IF object_id('RADB.dbo.CRD_ERAS_EncDim_GHGI') IS NOT NULL
	DROP TABLE RADB.dbo.CRD_ERAS_EncDim_GHGI;
...
SELECT
 ...
 INTO RADB.dbo.CRD_ERAS_EncDim_GHGI
FROM clarity.dbo.PAT_ENC_HSP AS peh 
   JOIN  RADB.dbo.CRD_ERAS_Case_GHGI AS ec ON ec.admissioncsn=peh.PAT_ENC_CSN_ID   
   LEFT JOIN clarity.dbo.HSP_ACCOUNT  AS hsp ON peh.HSP_ACCOUNT_ID=hsp.HSP_ACCOUNT_ID
   LEFT JOIN clarity.dbo.ZC_ACCT_BASECLS_HA AS basecl ON basecl.ACCT_BASECLS_HA_C=hsp.ACCT_BASECLS_HA_C
   LEFT JOIN clarity.dbo.ZC_MC_PAT_STATUS AS zcpat ON zcpat.PAT_STATUS_C=hsp.PATIENT_STATUS_C
   LEFT JOIN clarity.dbo.ZC_DISCH_DISP AS zdd ON zdd.DISCH_DISP_C=peh.DISCH_DISP_C
  ...
  UPDATE RADB.dbo.CRD_ERAS_EncDim_GHGI
  SET NumberofProcs=procct.ct
  FROM RADB.dbo.CRD_ERAS_EncDim_GHGI AS e
  LEFT JOIN 
  ( SELECT admissioncsn,COUNT(*) AS ct
    FROM radb.dbo.CRD_ERAS_Case_GHGI 
