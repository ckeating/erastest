...
FROM (			SELECT iln.*,f.log_id 					
			FROM    RADB.dbo.CRD_ERAS_Case_GH AS f
...
SELECT ifgd.FLO_MEAS_ID,ifgd.FLO_MEAS_NAME,DISP_NAME,
ifgd.MULTI_SEL_YN,
ValueType=zvt.NAME,
DUPLICATABLE_YN,
DUPLICATEABLE_YN,
...
FROM  clarity.dbo.IP_FLO_GP_DATA AS ifgd 
		...
		WHERE ifgd.FLO_MEAS_ID='8148'

SELECT * FROM dbo.IP_FLO_GP_DATA AS ifgd
LEFT JOIN dbo.IP_FLO_GP_DATA_2 AS ifgd2 ON ifgd2.FLO_MEAS_ID = ifgd.FLO_MEAS_ID
LEFT JOIN clarity.dbo.IP_FLO_CUSTOM_LIST AS ifcl
WHERE ifgd.FLO_MEAS_ID='8148'

SELECT * FROM 
clarity.dbo.IP_FLO_CUSTOM_LIST 
WHERE	 id='304668988'

SELECT *
FROM radb.dbo.CRD_ERAS_Case_GH AS cecg
SELECT 


SELECT * FROM radb.dbo.vw_PatEnc AS vpe
WHERE PAT_ENC_CSN_ID=128348725
...
FROM    clarity.dbo.IP_DATA_STORE AS ids
		...
		WHERE ids.EPT_CSN=126375990
		AND MEAS_VALUE LIKE '%compression%'
